...
import RubricTagPicker from "../../ui/RubricTagPicker";
import PhonesBlock from "../PhonesBlock";
import PhotosBlock from "../PhotosBlock";
import { DEFAULT_PHONE, DEFAULT_PHOTO, FUEL_TYPES, SERVICES, FEATURE_FUEL_NAME, FEATURE_SERVICE_NAME } from "../../../const";
...
const StationCard = ({ station, rubrics, networkRubric, onUpdate, onRemove, onDuplicate }: Props) => {
  ...
  return (
    <div className="card station-card">
      ...
      <div className="block" style={{ opacity: 0.6, pointerEvents: "none" }}>
        <strong>Характеристики (feature-enum-multiple)</strong>
        <p className="field-hint" style={{ color: "var(--color-warning, #b45309)", fontWeight: 500 }}>
          ⚠ Необходимы актуальные <code>name</code> из импорта
        </p>
        <p className="field-hint">
          Атрибут <code>name</code> тега <code>&lt;feature-enum-multiple&gt;</code> индивидуален для каждой организации
          и может отличаться от захардкоженных значений (<code>{FEATURE_FUEL_NAME}</code>, <code>{FEATURE_SERVICE_NAME}</code>).
          <br /><br />
          <strong>Как получить актуальные name:</strong>
          <br />
          1. Войдите в <strong>Яндекс Бизнес</strong> (business.yandex.ru) в аккаунт организации.
          <br />
        ...
        <label style={{ marginBottom: 8 }}>
          ...
          <div className="rubric-tags">
            {FUEL_TYPES.map((fuel) => (
              <span className="rubric-tag" key={fuel} style={{ cursor: "default" }}>{fuel}</span>
            ))}
          </div>
        </label>
        <label>
          <span className="label-title">Услуги ({FEATURE_SERVICE_NAME})</span>
          <div className="rubric-tags">
            {SERVICES.map((service) => (
              <span className="rubric-tag" key={service} style={{ cursor: "default" }}>{service}</span>
            ))}
          </div>
        </label>
      </div>
